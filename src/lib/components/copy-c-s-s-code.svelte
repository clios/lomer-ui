<script lang="ts">
    ...
    let { class: className, code, title = 'CSS Code', highlightedLines, startingLineNumber }: Props = $props();
...
<div class={twMerge('relative mt-2 flex flex-col border border-dotted text-sm sm:text-base', className)}>
    <div class="bg-fg dark:bg-bg text-bg dark:text-fg flex items-center justify-between py-1.5 pr-1.5 pl-2.5">
        <p>{title}</p>
    ...
    <div class="overflow-hidden">
        <HighlightSvelte lang={css} {code} let:highlighted>
            <LineNumbers
                {highlighted}
                {highlightedLines}
                {startingLineNumber}
