...
{#if open}
  <div
    class={twMerge(
      'relative flex items-start gap-3 p-3 w-full', // layout & positioning
  ...
  >
    ...
    <!-- CLOSE BUTTON -->
    {#if closable}
      <button class="top-3 right-3 absolute cursor-pointer" onclick={() => (open = false)}>
        {@render IconClose()}
      </button>
    {/if}
  </div>
...
{#snippet IconInfo()}
  <svg class="min-w-6 size-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
    <path
      fill="currentColor"
