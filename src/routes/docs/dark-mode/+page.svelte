...
<div class="sticky top-0">
	<aside class="absolute right-0 flex w-64 flex-col gap-2 text-sm">
		...
		<button on:click={() => scrollToId('0')} class="text-left font-semibold"
			>Usage</button
		>
		<button on:click={() => scrollToId('1')} class="ml-2 text-left"
			>Install mode-watcher</button
		>
		<button on:click={() => scrollToId('2')} class="ml-2 text-left"
			>Add ModeWatcher component</button
		>
		<button on:click={() => scrollToId('3')} class="ml-2 text-left"
			>That's it</button
		>
	</aside>
</div>
...
<div class="pr-80">
	...
	<div class="ml-2 flex flex-col gap-2 border-l pb-8 pl-6 dark:border-zinc-700">
		<p>
			Add the <span class="rounded bg-zinc-200 px-1 dark:bg-zinc-800"
				>{`<ModeWatcher />`}</span
			>
			component to your root
			<span class="rounded bg-zinc-200 px-1 dark:bg-zinc-800"
				>+layout.svelte</span
			> file.
		</p>
		<Code
	...
	<div class="ml-2 flex flex-col gap-2 pb-8 pl-6 dark:border-zinc-700">
		<p>
			Here's an example of how to use the <span
				class="rounded bg-zinc-200 px-1 dark:bg-zinc-800">toggleMode</span
			>
			function and
			<span class="rounded bg-zinc-200 px-1 dark:bg-zinc-800">$mode</span> to toggle:
		</p>
